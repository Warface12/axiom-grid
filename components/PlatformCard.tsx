...
export function PlatformCard({ platform }: { platform: Platform }) {
  ...
  return (
    <article className="platform-card">
      ...
      <div>
        <span className="kind-label">{platform.kind}</span>
        <h3>{platform.name}</h3>
        <p>{platform.short}</p>
      </div>
      ...
      <div className="platform-card-foot">
        <span>Updated {platform.updatedAt}</span>
        <Link href={`/${platform.kind}s/${platform.slug}`}>Research profile <ArrowUpRight size={15} /></Link>
      </div>
    </article>
  );
}
