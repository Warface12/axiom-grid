...
const empty = { ok: true, user: null, items: [], follows: [], notifications: [], preferences: null, interests: [] };
...
export async function GET() {
  try {
    ...
    const db = adminDb();
    if (!db) return NextResponse.json({ ...empty, ok: true, user: { email: user.email } });
    const profile = await ensureUserProfile(db, user);
    if (!profile) {
      return NextResponse.json({ ...empty, user: { email: user.email, id: user.id } });
    }
    ...
    logAuthFailure("account.me.get", error);
    return NextResponse.json({ ...empty, ok: true, unavailable: true, error: publicAuthMessage(error) });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const db = adminDb();
    if (!db) return NextResponse.json({ ok: false, error: "Your account is signed in, but the profile service is unavailable." }, { status: 503 });
    ...
    logAuthFailure("account.me.post", error);
